...
class Player(object):
	...
	def updateSpeed(self, keyInputs, aiInput):
		#switch between user or AI input
		if(keyInputs == []):
			if aiInput==1:
			...
			if keyInputs[K_UP] or aiInput==1:
				self.speed = -self.init_speed

			if keyInputs[K_DOWN] or aiInput==2:
				self.speed = self.init_speed

			if not keyInputs[K_DOWN] and not keyInputs[K_UP] and aiInput==0:
				self.speed = 0
